...
interface WechatResolvedHtml {
	htmlContent: string;
	contentHtmlForCover: string;
}
...
export default class WechatAccountService extends AbstractAccountService {
	...
	private injectWechatHeaderFooterSlots(
		...
		const articleForSlotInjection = footerOverride
			? {
				...article,
				contentSlots: {
					...(article.contentSlots ?? {}),
					footerHtml: footerOverride,
				},
			}
			: article;
		return applyHtmlContentSlots(htmlContent, articleForSlotInjection, {
	...
	private async resolveArticleHtml(article: SharedArticle): Promise<WechatResolvedHtml> {
		...
		const footerHtml = contentSlots.footerHtml || this.buildWechatDefaultFooterHtml();
		const hasFooterSlot = Boolean(footerHtml);
		const footerInsertion = !hasFooterSlot
			? "not_configured"
			: sourceHtml.includes(WECHAT_FOOTER_SLOT_PLACEHOLDER)
				|| sourceHtml.includes(FOOTER_SLOT_PLACEHOLDER)
					? "replaced_placeholder"
					: "appended_at_end";
		const footerImageScanPlaceholder = hasFooterSlot
			? createFooterImageScanPlaceholder(sourceHtml, footerHtml, "wechat")
			: "";
		htmlContent = this.injectWechatHeaderFooterSlots(
			article,
			htmlContent,
			footerImageScanPlaceholder || undefined,
		);
		...
		let replacedHtml = this.replaceHtmlImageUrlsByMap(
			htmlContent,
			(rawUrl) => this.normalizeImageUrl(rawUrl),
			this.imageUrlCache,
		);
		const finalImageSources = this.extractImageUrlsFromHtmlContent(replacedHtml);
		const replacedContentImages = imageSources.filter((source) => {
			const normalized = this.normalizeImageUrl(source);
			return Boolean(
				normalized
				&& !this.isWechatHostedImage(normalized)
		...
		const contentHtmlForCover = replacedHtml;
		...
		await this.tracePublish({
			...
			metadata: {
				...
				finalHtmlImages: finalImageSources.length,
	...
	async articleDraft(article: SharedArticle): Promise<ArticleDraft | null> {
		try {
			...
			const resolved = await this.resolveArticleHtml(article);
			const resolvedHtml = resolved.htmlContent;
			const htmlWithStyle = this.injectWechatStyleSheet(resolvedHtml);
			const digest = this.resolveDigest(article, resolvedHtml);
			const thumbMedia = await this.resolveThumbMedia(article, resolved.contentHtmlForCover);
