		...
		<section className="rounded-xl border border-design-border bg-white p-3.5">
			<div className="mb-2.5 flex items-center justify-between gap-3">
				<div className="flex min-w-0 items-center gap-3">
					...
					<p className="min-w-0 truncate font-display text-[15px] font-semibold text-design-text">{title}</p>
				</div>
		...
		<div className="mt-3 space-y-3">
			...
			>
				...
				<button
					...
					disabled={coverUploadDisabled}
					onClick={openCoverPicker}
					className="group relative block aspect-[21/9] w-full overflow-hidden rounded-xl border border-dashed border-design-border bg-design-background text-left transition-colors hover:border-brand-300 hover:bg-brand-50/40 disabled:cursor-not-allowed disabled:hover:border-design-border disabled:hover:bg-design-background"
				>
					{article?.coverImage ? (
						<>
							<img src={article.coverImage} alt="封面预览" className="h-full w-full object-contain" />
							<div className="absolute inset-0 flex items-center justify-center bg-slate-950/0 text-[12px] font-medium text-white opacity-0 transition-all group-hover:bg-slate-950/35 group-hover:opacity-100">
								点击更换封面
							</div>
						</>
					) : (
						<div className="flex h-full flex-col items-center justify-center gap-2 px-4 text-center text-[12px] text-design-textSecondary">
							{loading.uploadCover ? (
								<Loader2 className="h-5 w-5 animate-spin text-brand-500" />
							) : (
								<Upload className="h-5 w-5 text-design-neutral transition-colors group-hover:text-brand-500" />
							)}
				...
				{uploadMessage && (
					<div className="mt-3 flex items-center gap-2 rounded-xl border border-emerald-200 bg-emerald-50 px-3 py-2 text-[12px] text-emerald-700">
			...
			>
				<Textarea
					...
					placeholder="输入文章摘要"
					className="min-h-[84px] resize-none rounded-md border-design-border bg-white p-3 text-[13px] leading-6"
					rows={3}
				/>
			</FeatureCard>
