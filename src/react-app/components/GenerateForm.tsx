	...
	return (
		<div className="space-y-4">
			<div className="space-y-2">
				<label className="text-[13px] font-semibold text-design-text">标题（可选）</label>
				<Input value={value.title} onChange={(event) => onChange({ ...value, title: event.target.value })} placeholder="留空则按热门标题生成" />
			</div>
			<div className="space-y-2">
				<label className="text-[13px] font-semibold text-design-text">标题来源</label>
			...
			{value.titleSource === "custom" ? (
				<div className="space-y-2">
					<label className="text-[13px] font-semibold text-design-text">自定义标题（每行一个）</label>
					<Textarea value={value.customTitles} onChange={(event) => onChange({ ...value, customTitles: event.target.value })} />
				</div>
			) : (
				<div className="space-y-2">
					<label className="text-[13px] font-semibold text-design-text">热门标题样本</label>
					<Textarea value={value.sourceTitles.join("\n")} readOnly placeholder="点击获取热门标题" />
				</div>
			)}
			<div className="space-y-2">
				<label className="text-[13px] font-semibold text-design-text">大纲（可选）</label>
				<Textarea value={value.outline} onChange={(event) => onChange({ ...value, outline: event.target.value })} />
			</div>
			<div className="grid gap-4 md:grid-cols-2">
				<div className="space-y-2">
					<label className="text-[13px] font-semibold text-design-text">语气</label>
				...
				<div className="space-y-2">
					<label className="text-[13px] font-semibold text-design-text">长度</label>
